...
def diagnose_single_model(model_path, render=False):
    ...
    if stats['is_collapsed']:
        ...
        print(f"   The agent uses action {stats['dominant_action']} ({action_names.get(stats['dominant_action'])}) {stats['dominant_pct']:.1f}% of the time")
...
def find_healthy_checkpoint(model_dir, max_checkpoints=10):
    ...
    for ckpt in reversed(checkpoints_to_test):
        ...
        try:
            ...
            print(f"  Dominant: {stats['dominant_pct']:.1f}%, X_pos: {stats['max_x_pos']}, Entropy: {stats['entropy']:.3f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default=None,
                        help='Path to specific model to diagnose')
    parser.add_argument('--find-healthy', action='store_true',
                        help='Find the most recent healthy checkpoint')
    parser.add_argument('--model-dir', type=str, default='./mario_models',
                        help='Directory containing model checkpoints')
    parser.add_argument('--render', action='store_true',
                        help='Render the game while diagnosing')
    
    args = parser.parse_args()
    
    if args.model:
        diagnose_single_model(args.model, render=args.render)
    elif args.find_healthy:
        find_healthy_checkpoint(args.model_dir)
    else:
        # Default: diagnose best model
        best_model = os.path.join(args.model_dir, 'best', 'best_model.zip')
        if os.path.exists(best_model):
            diagnose_single_model(best_model, render=args.render)
        else:
            print("No best model found. Use --model to specify a model path or --find-healthy to scan checkpoints.")
