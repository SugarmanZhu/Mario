...
class PolicyCollapseCallback(BaseCallback):
    ...
    def _find_checkpoint_before_step(self, step: int) -> Optional[str]:
        ...
        pattern = os.path.join(self.checkpoint_dir, f"{self.checkpoint_prefix}*_steps.zip")
        checkpoints = glob.glob(pattern)
    ...
    def _on_step(self) -> bool:
        ...
        try:
            ...
            is_collapsed = (
                dominant_ratio > self.dominant_action_threshold or 
            ...
            if is_collapsed:
                ...
                if self.verbose > 0:
                    ...
                    print(f"    Dominant action: {dominant_action} ({dominant_ratio*100:.1f}%)")
                    print(f"    Entropy: {entropy:.4f}")
                    print("!" * 60)
                
                # Try to recover from last healthy checkpoint
                if self.last_healthy_checkpoint and os.path.exists(self.last_healthy_checkpoint):
                    if self.verbose > 0:
                ...
                if current_checkpoint and current_checkpoint != self.last_healthy_checkpoint:
